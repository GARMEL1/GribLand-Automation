...
#include <cstring> // Для функции stricmp
...
void scriptThread(const DWORD& targetProcessId) {
...
int main() {
    ...
    // Создание потока для выполнения основной логики программы
    std::thread script(scriptThread, targetProcessId);

    // Основной поток продолжает выполнение, пока поток скрипта выполняет свою работу
    script.detach();
